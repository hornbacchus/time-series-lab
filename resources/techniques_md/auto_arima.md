...
**The selected order is a starting point, not a verdict.** The (p, d, q) auto_arima reports — and, for a seasonal fit, the (P, D, Q) — is chosen by pmdarima's stepwise search: it walks a restricted set of candidate models and keeps the one that minimizes an information criterion. That search is a data-driven heuristic. On finite samples it can over- or under-select, and different automatic routines (for example R's `auto.arima`) will not always land on the same order for the same series. What is validated here is the fit produced for a given order — not the choice of order itself. So where you carry a structural prior — a known seasonal period, an expected order of differencing, a familiar (p, q) for the series — specify it and compare, rather than treating the auto-selected order as canonical.
