...
## 7. Resolution (source-read; record-only, no engine/check change)

The FUND-OR-DISCLOSE / disclose-terminal bands were read-and-decided. Outcome: **disclose, no new checks.**

**(A) `quantile_regression` — R quantreg INFEASIBLE (the band's one cross-package hope, closed).** The engine is sklearn `GradientBoostingRegressor(loss="quantile")` — gradient-boosted TREE-ENSEMBLE quantile regression, **NOT linear QR**. R `quantreg::rq` (linear pinball) is a **different model class** → cannot reference it (the forecastHybrid / R-BVAR pattern). Honesty precision: a cross-implementation GBR-quantile reference (R gbm/xgboost-quantile) is **judged** unlikely to match (implementation-specific tree construction) — a judgment, not a tested result. Existing check is engine-invoked self-parity (honest). Terminal-disclosed.

**(B) ML/DL deep-learning block (14) — already honestly self-parity-disclosed (integrity scan 0/14 overstate).** No off-the-shelf independent reference exists (the engine IS the canonical impl; same-lib trap); validation = self-parity + honest disclosure = the **correct terminal tier** (functional checks would be apparatus barely above disclosure — not built). prophet = none-intrinsic (Stan). ★ `transformer_forecast` is the **weakest**: no full-technique `p3_transformer` check, only the sub-component `3f_transformer_attention` — sub-component-validated only.

**(C) disclose-terminal band (~46 weak rows) — integrity scan: ~0% mislabel; NO blanket pass.** A Group-2 sample (0/15 overstate) shows the weak tier is **already honestly tiered** — genuine-but-weak cross-package (arima/sarima/ets_hw/theta vs R forecast; garch family vs rugarch; har_rv vs R lm), honest CAVEAT (emd_hht/mstl/stl, Tier VI), correct SKIP (x13). A 46-row rewrite is **redundant**.

★ **Meta-finding (why the weak band needs no further scrutiny):** the map's overstatement pattern was **STRONG-TIER-BOUNDED** — all 4 digest errors (conformal base, nar_narx, var, vecm) + the hollow-strong entries were *recorded-strong* (where there was something to overstate). The weak/terminal tier is honestly labelled (nobody mislabels self-parity as self-parity). So "the map overstates" is REAL but BOUNDED to the strong tier — now fully audited and corrected — and does **not** generalize to the weak band. This bounds the remaining audit surface to zero.
