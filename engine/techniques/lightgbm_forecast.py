...
def run(ctx: RunContext, progress_callback) -> dict:
    ...
    try:
        ...
        # CAI Phase 2 Session 23 fix (F-TR-LGBM-HORIZON): explicit
        # range gate. Pre-fix, horizon<1 silently coerced to 1.
        if horizon < 1:
            return make_error_response(
                ctx,
        ...
        # CAI Phase 2 Session 23 fix (F-TR-LGBM-NLAGS): explicit
        # range gate.
        if n_lags < 1:
            return make_error_response(
                ctx,
