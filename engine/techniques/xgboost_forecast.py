"""
XGBoost Time Series Forecast for Time Series Lab.

Uses the XGBoost library when available, falling back to sklearn
...
def run(ctx: RunContext, progress_callback) -> dict:
    """
    XGBoost time series forecast with lag features.
    ...
    try:
        ...
        if n < 20:
            return make_error_response(
                ...
                "XGBoost forecast needs at least 20.",
        ...
        use_xgb = _has_xgboost()
        backend = "xgboost" if use_xgb else "sklearn_gbr"

        if not use_xgb:
            warn_list.append(
                "XGBoost library not available. Falling back to sklearn "
                "GradientBoostingRegressor. Install xgboost for full support."
            )

        if use_xgb:
            import xgboost as xgb

            progress_callback("Training XGBoost model", 25)
            ...
            model_desc = "XGBoost (xgboost library)"
        else:
        ...
        plain_english = (
            f"XGBoost forecast for '{name}' ({n} observations) using {backend} with "
        ...
        try:
            ...
            def build_interpretation(technique_id, results):  # type: ignore
                return None
        interp = build_interpretation("xgboost_forecast", dict(audit))
