"""
Gradient Boosting Forecast for Time Series Lab.

Uses sklearn GradientBoostingRegressor with automatically generated
...
def run(ctx: RunContext, progress_callback) -> dict:
    """
    Gradient boosting forecast with lag features.
    ...
    n_estimators : int, optional
        Number of boosting iterations. Default from preset.
    max_depth : int, optional
        Maximum tree depth. Default from preset.
    learning_rate : float, optional
        Boosting learning rate. Default from preset.
    """
    try:
        ...
        if n < 20:
            return make_error_response(
                ...
                "Gradient boosting needs at least 20.",
                error_fixes=["Provide a longer time series."],
            )

        horizon = int(ctx.get_param("horizon", 10))
        # CAI Phase 2 Session 23 fix (F-TR-GBM-HORIZON): explicit
        # range gate. Pre-fix, horizon<1 was silently coerced to 1.
        if horizon < 1:
            return make_error_response(
                ctx,
        ...
        progress_callback("Training gradient boosting model", 25)
        ...
        plain_english = (
            f"Gradient boosting forecast for '{name}' ({n} observations) with "
        ...
        try:
            ...
            def build_interpretation(technique_id, results):  # type: ignore
                return None
        interp = build_interpretation("gradient_boosting_forecast", dict(audit))
        ...
        return make_error_response(
            ctx,
            f"Gradient boosting forecast failed: {e}",
            error_fixes=[
