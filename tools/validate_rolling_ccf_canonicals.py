"""Phase 5 canonical validation for rolling_ccf_lag.
...
def _build_ctx(x, y, *, params=None):
    return RunContext({
        "run_id": "test_rccf", "technique_id": "rolling_ccf_lag",
...
def canonical_1():
    print("\n=== c1: rccf baseline ===")
    x, y = _lagged()
    res = rccf_mod.run(_build_ctx(x, y, params={"max_lag": 20}), _null)
    return res.get("status") == "success"


def canonical_2():
    print("\n=== c2: rccf with explicit window ===")
    x, y = _lagged(seed=43)
    res = rccf_mod.run(_build_ctx(x, y, params={"max_lag": 20,
                                                   "window": 100}), _null)
    return res.get("status") == "success"


def canonical_3():
    print("\n=== c3: rccf real (DGS2, DGS10) ===")
    ...
    n = min(len(x), len(y))
    res = rccf_mod.run(_build_ctx(x[-n:], y[-n:],
                                     params={"max_lag": 20}), _null)
    return res.get("status") == "success"


def canonical_4():
    print("\n=== c4: rccf step variant ===")
    x, y = _lagged(T=600, seed=44)
    res = rccf_mod.run(_build_ctx(x, y, params={"max_lag": 20,
                                                   "step": 5}), _null)
    return res.get("status") == "success"
...
def canonical_5():
    print("\n=== c5: rccf independent series ===")
    rng = np.random.default_rng(45)
    res = rccf_mod.run(_build_ctx(rng.standard_normal(400).tolist(),
...
def canonical_6():
    print("\n=== c6: rccf short series ===")
    x, y = _lagged(T=80, seed=46)
    res = rccf_mod.run(_build_ctx(x, y, params={"max_lag": 10}),
                         _null)
