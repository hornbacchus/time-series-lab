"""Phase 5 canonical validation for cross_correlation_lag.
...
def _build_ctx(x, y, *, params=None):
    return RunContext({
        "run_id": "test_ccf", "technique_id": "cross_correlation_lag",
...
def canonical_1():
    print("\n=== c1: ccf baseline ===")
    x, y = _lagged()
    res = ccf_mod.run(_build_ctx(x, y, params={"max_lag": 20}), _null)
    return res.get("status") == "success"


def canonical_2():
    print("\n=== c2: ccf normalize toggle ===")
    x, y = _lagged(seed=43)
    for norm in [True, False]:
        res = ccf_mod.run(_build_ctx(x, y, params={"normalize": norm,
                                                      "max_lag": 20}), _null)
...
def canonical_3():
    ...
    n = min(len(x), len(y))
    res = ccf_mod.run(_build_ctx(x[-n:], y[-n:],
                                   params={"max_lag": 20}), _null)
    return res.get("status") == "success"


def canonical_4():
    print("\n=== c4: ccf identical series (lag=0) ===")
    rng = np.random.default_rng(44)
    z = rng.standard_normal(300).tolist()
    res = ccf_mod.run(_build_ctx(z, z, params={"max_lag": 10}), _null)
    return res.get("status") == "success"


def canonical_5():
    print("\n=== c5: ccf short series T=50 ===")
    rng = np.random.default_rng(45)
    res = ccf_mod.run(_build_ctx(rng.standard_normal(50).tolist(),
...
def canonical_6():
    print("\n=== c6: ccf independent series ===")
    rng = np.random.default_rng(46)
    res = ccf_mod.run(_build_ctx(rng.standard_normal(400).tolist(),
                                   rng.standard_normal(400).tolist(),
                                   params={"max_lag": 20}), _null)
    return res.get("status") == "success"
