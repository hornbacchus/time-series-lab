...
**Last updated:** 2026-04-29 (**Phase 3.5 Session 2 close** — Item 8: 12 pre-Phase-3 inherited checks migrated from `ParityCheck` to `P3ParityCheck` contract per S5 lock. All 82 active checks now satisfy P-1 §8.1 (`verdict_class` + `verdict_class_rationale` declared on every check). 11-class verdict_class taxonomy validated empirically — no new class needed. Fast-tier 76/76 unchanged from Phase 3 close (71 PASS + 5 CAVEAT, 0 BLOCK). Slow-tier checks compile clean post-migration; runtime verification deferred to next nightly. Banked: structural_invariants declarations for 12 inherited checks deferred to Phase 3.5 Session 9 candidate (would require run_tsl modifications to populate invariant input keys; out of Item 8 scope).
