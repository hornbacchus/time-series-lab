...
**Version:** v1.2.0 (issued at Phase 4 Session 12c-2, 2026-05-03; v1.1.0 at Phase 3.5 Session 11, 2026-04-30; v1.0.0 at Phase 3 Session 17)
...
**Update (Phase 4 Session 12c-2 v1.2.0 issuance):** the
first concrete DD verdict instance landed at **Phase 4
Session 5** (BYF candidate #1 R `BVAR::bvar()` Pattern A.2
audit; `max_rel_diff=1.76` on Minnesota posterior). A
second DD instance landed at Phase 4 Session 6 (BYF #3
stochvol partial A.2). See P-3 §3.4.3 (BVAR DD finding)
+ §3.4.4 (stochvol partial DD finding) for the cycle-close
...
### 6.10 — Phase 4 cycle close consolidation (S12c-2 v1.2.0 issuance)

The Phase 4 cycle closes with the **13-item inheritance
register fully dispositioned** (12 closed in-cycle + BYF
#5 closed at S11c-2 = 13/13 closed) AND **2 forward-banked
observations** explicitly deferred to Phase 4.5+
(B-Phase4-S7-1 None-handling bug; B-Phase4-S10-3 smoke-
test n_draws insufficiency).
...
- **B-Phase4-S7-1** — None-handling bug in 6 concrete
  invariant checkers. Surfaced at S7 P4-1.1 registry
  expansion; per §11.8 blast-radius discipline NOT fixed
  within S7; banked for Phase 4.5+ runner-integration
  session.
...
explicitly-designed forward-banked items deferred to
Phase 4.5+. No items had to be banked forward as silent
overruns; the cycle closed with the cleanest cycle-close
state achievable per B-Phase4-S11c-2-1 institutional
precedent.
...
**Phase 4 closure (S12c-2 v1.2.0 issuance update):** P4-1
...
**Phase 4 closure (S12c-2 v1.2.0 issuance update):** P4-2
...
**Phase 4 closure (S12c-2 v1.2.0 issuance update):** P4-3
...
| **1.2.0** | **2026-05-03** | **Claude Code (Phase 4 Session 12c-1 + S12c-2 — closes v1.2.0 doc-set issuance event)** | **Phase 4 cycle close amendments — P-3 v1.2.0 ISSUED.** (§3.4.1 NEW; S11a-1) O-1 banking near-unit-root VAR companion margin observation. (§3.4.2 NEW; S11a-3) DD forward-provisioning interval (~6 months Phase 3.5 S1 wiring → Phase 4 S5 first runtime). (§3.4.3 NEW; S5; S12c-1) Phase 4 BVAR DD finding — first DD outcome in TSL parity history (BYF #1; max_rel_diff=1.76 on Minnesota; methodology gap CCM-2019 Minnesota vs R BVAR hierarchical Litterman). (§3.4.4 NEW; S6; S12c-1) Phase 4 stochvol partial DD finding — second DD outcome (BYF #3; CCM-KSC vs standalone svsample). (§3.4.5 NEW; B-Phase4-S6-1 cross-doc P-3 side; S12c-1) Auto-DD pattern empirical-findings-side codification per Disposition 3. (§6.6 update; S12c-2) DD verdict reservation FIRST RUNTIME at Phase 4 S5; cross-references to P-3 §3.4.3/§3.4.4/§3.4.2. (§6.10 NEW; S12c-2) Phase 4 cycle close consolidation — 13-item inheritance register fully dispositioned; 2 forward-banked items explicit deferral (B-Phase4-S7-1; B-Phase4-S10-3). (§7.1/§7.2/§7.3 closure dispositions; S12c-2) P4-1 CLOSED at S7+S8+S9; P4-2 CLOSED at S2; P4-3 CLOSED at S3. (§8 numbering fix; S12c-2 per Disposition 1) Subsections renumbered §7.1/§7.2/§7.3 → §8.1/§8.2/§8.3 (pre-existing Phase 3.5 v1.1.0 amendment artifact corrected). |

---

**End of Parity Empirical Findings P-3 v1.2.0.**

**Phase 4 v1.2.0 doc-set issuance event COMPLETE for P-1 +
P-2 + P-3.** All 4 docs at Phase 4 target versions: P-1
v1.2.0 (S12a) + P-2 v1.2.0 (S12b-2) + P-3 v1.2.0 (S12c-2,
this commit) + C-1 v2.0.0 (Phase 4 S10). Session 13 (P-4
v1.2.0 + Phase 4 cycle close) is the final Phase 4 session.
